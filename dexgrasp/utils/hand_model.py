...
class HandModel:
    def __init__(self, mjcf_path, mesh_path, contact_points_path, penetration_points_path, n_surface_points=0, device='cpu',use_joint21=False):
        ...
        # 0->root, 1-4->index, 5-8->middle,9-12->ring, 13-16->litttle, 17-20(19)->thumb
        if use_joint21:
            penetration_points['robot0:palm'] = [0,0,0.01]
            thproximal_p0 = penetration_points['robot0:thproximal_child']
            penetration_points['robot0:thproximal_child'] = [[0,0,0.005],thproximal_p0]
            a=1
        # build mesh
                ...
                for visual in body.link.visuals:
                    ...
                    if visual.geom_type == "box":
                        ...
                        if visual.geom_param[1] is not None:
                            scale = torch.tensor(visual.geom_param[1], dtype=torch.float, device=device).clone().detach()
    ...
    def get_surface_points(self):
        ...
        for link_name in self.mesh:
            ...
            points_part_num.append(n_surface_points)
    ...
    def get_trimesh_data(self, i, opacity=0.5, color='pink'):
        """
        ...
        """
        color_rgb = np.asarray(mcolors.to_rgb(color))*254
        ...
        for link_name in self.mesh:
            ...
            mesh_i=tm.Trimesh(vertices=v, faces=f,vertex_colors=np.ones_like(v)*color_rgb)
            data.append(mesh_i)
