...
class DemoReference:
    ...
    align_rotation 기본값=False (8/25 확정): G3 실측 결과 물체 회전까지 맞추는 쪽이
    오히려 더 나빴다(위치만 정렬 23.8% vs 위치+회전 정렬 4.8%, demo_replay_batch.py).
    obj_rotmat이 물체의 "의미 있는 방향"이 아니라 시도마다 거의 무작위값이라
    (verify_demo_convention.py: 같은 오브젝트 안에서도 ||ΔR||_F 0.77~2.83), 그걸 기준으로
    회전을 맞추면 오히려 원래 월드 프레임에서 그럭저럭 맞던 접근 각도를 망가뜨린다.
    """

    def __init__(self, demo, hand_root_offset, align_rotation=False):
        """demo: record_demo.py가 저장한 dict
        hand_root_offset: (3,) palm_pos_world - virtual_xyz 상수 c
        align_rotation: True면 물체 회전까지 맞춰 정렬(비권장, 실측상 더 나쁨).
                        False(기본)면 위치만 정렬 - 손목 회전은 월드 프레임 기준 그대로 둠."""
        ...
        # 시연 당시 물체 초기 pose 기준으로 정렬 (align_rotation=False면 항등회전 사용
        # = 물체 위치만 빼고 회전은 그대로 두는 것과 동일)
    ...
    def warp(self, obj_pos, obj_quat, actions=None, finger_limits=None):
        ...
        # align_rotation=False면 물체 회전은 안 곱하고 위치만 더한다(월드 프레임 접근각 유지).
        if self.align_rotation:
            ...
            palm_pos_new = pos_w + obj_pos[None, :, :]
            palm_quat_new = np.broadcast_to(quat_w, (T, N, 4)).copy()
